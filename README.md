...
demonstrate how to provide Worktips related JSON REST service. For this purpose,
 a service called Open Worktips was developed.


OpenWorktips is an open source implementation of backend of
https://myworktips.com/. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://myworktips.com/.

Open Worktips's backend is open sourced, free
to use, host and modify. Additionally, some features were added/changed as compared
to MyWorktips. They include:

 - google analytics, cloudflare, images and flash were removed.
 - frontend's html and javascript upgrades and cleanups.
 - transaction service fee was set to zero (same as in MyWorktips).
 - the wallets generated use 25 word mnemonics, fully compatible with official worktips wallets
(13 word mnemonics generated by MyWorktips work as usual though).
...
![Open Worktips](https://raw.githubusercontent.com/worktipsexamples/openworktips/master/screenshot/screen1.png)


## Host it yourself

The Open Worktips consists of four components that need to be setup for it to work:
 ...
 - Frontend - it is virtually same as that of MyWorktips, except before mentioned differences.
  It consists of HTML, CSS, and JavaScript.
 - Worktips daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Open Worktips into the Worktips network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Worktips blockchain and "talks"
 with Worktips deamon.
...
Open Worktips is not as fast as MyWorktips.
...
Below are example and basic instructions on how to setup up and run Open Worktips on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### Worktips download and compilation

To download and compile recent Worktips follow instructions
in the following link:

https://github.com/worktipsexamples/worktips-compilation/blob/master/README.md

#### Compilation of the OpenWorktips (don't run it yet)

Once Worktips was downloaded and compiled, we can download Open Worktips and compile it.
...
work without database, setup frontend, and synced and running worktips blockchain.
...
Create openworktips database called `openworktips`.

```
cd openworktips/sql
docker exec -i ommariadb mysql -uroot -proot < openworktips.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the openworktips database is through the
...
```
docker run --name omhtml -p 80:80 -v /home/mwo/openworktips/html:/usr/share/nginx/html:ro -d nginx
```

where `omhtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/mwo/openworktips/html` is the location on your host computer where the
...
#### myworktips-core-js (optional)

OpenWorktips uses frontend code provided by myworktips.com. Among many files
used, the two crtical ones are binary webassamply
[MyWorktipsCoreCpp_WASM.wasm](https://myworktips.com/js/lib/myworktips_core_js/worktips_utils/MyWorktipsCoreCpp_WASM.wasm) and
the corresponding JavaScript [myworktips-core.js](https://myworktips.com/js/lib/myworktips-core.js) files.
They are used by [send_coins.js](https://myworktips.com/js/controllers/send_coins.js?) for providing
transaction generation functionality.

OpenWorktips provides these files here: `./html/js/lib`. They were generated using forked  `myworktips-core-js` repo:
https://github.com/worktipsexamples/myworktips-core-js/tree/openworktips

However, you can compile them yourself using the orginal repository located at
https://github.com/myworktips/myworktips-core-js.

Below are instructions on how it can be done on Arch Linux.

```
git clone https://github.com/myworktips/myworktips-core-js.git

cd myworktips-core-js/
...
# compile myworktips-core-js
./bin/build-emcpp.sh

# generate myworktips-core.js and MyWorktipsCoreCpp_WASM.wasm 
./bin/package_browser_js

```

The above instructions should produce `myworktips-core.js`
and `myworktips_core_js/worktips_utils/MyWorktipsCoreCpp_WASM.wasm`
(both located in `./build` folder), which can
be used in place the files bundled with OpenWorktips.
...
openworktips, Open Worktips backend service:
  ...
  -c [ --config-file ] arg (=./config/config.json)
                                        Config file path.
  -m [ --worktips-log-level ] arg (=1)    Worktips log level 1-4, default is 1.
  -l [ --log-file ] arg (=./openworktips.log)
...
Before running `openworktips`:
 ...
 - make sure worktips daemon is running and fully sync. If using testnet or stagenet networks, use worktips daemon
...
```bash
./openworktips -s -c /path/to/config.json
```


## OpenWorktips JSON REST API

Example JSON REST requests and their responses of [OpenWorktips](https://github.com/worktipsexamples/openworktips) are provided below. The long term goal
...
the OpenWorktips api does not fully conform to that.

#### get_version

Get version of the OpenWorktips, its API and worktips.
...
```json
{
  ...
  "worktips_version_full": "0.10.3.1-125f823"
}
```
...
Login an existing or a new user into OpenWorktips.
...
Get random outputs from the worktips daemon to be used as ring members
...
Submit raw tx in hex generated by the fronted to be relayed to worktips network.
